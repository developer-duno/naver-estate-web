...
describe("useMbRadarSettings 훅", () => {
  ...
  it("초기 상태에서 모든 축이 활성화되고 가중치가 3이다", async () => {
    ...
    expect(result.current.enabledAxes.size).toBe(9);
  ...
  it("toggleAxis로 축을 비활성화하면 localStorage에 저장된다", async () => {
    ...
    expect(result.current.enabledAxes.size).toBe(8);
  ...
  it("최소 3개 축 미만으로 비활성화할 수 없다", async () => {
    ...
    // 6개 비활성화 → 3개 남음
    const toDisable = ["units", "parking", "maxFloor", "jeonse", "nearby", "discount"];
