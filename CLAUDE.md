...
| **BE 깊이 토픽 1종** | `backend/.claude/details.md` | BE 작업 시 명시 참조 (자동 로드 안 됨) — 실거래가·mibunyang·검증·중복 제거·**스케줄러 잡 상세 6절**(infra.md 표에서 이동, 세션 411) |
...
| BE pytest | `backend/tests/` | **1869개** (세션 410 실측 `--collect-only` — #530 기준 1867(+2: explain_error 결제 상태별·공백 입력) → 후속 PR(마침표·마스킹·포괄 규칙 회귀 3건) 1869. 세션 409 값 1865 는 `702b01d`(#527) 기준, 1845 대비 +20. 알림 8창구 우리말화 회귀: 필드 사전·문구 경계(`test_field_drift_monitor.py`)·결제/수집 안내 분기와 조사(`test_job_error_listener.py`)·개발자 어휘/접두어 **전수 추출 가드**(`test_plain_words.py`, **워크플로 YAML 포함**). ⚠ **`--collect-only` 는 반드시 `backend/` 안에서 실행할 것** — 레포 루트에서 돌리면 `test_scheduler_monitoring_coverage.py` 가 `FileNotFoundError` 로 수집 오류를 내 "1824건 + 오류 1" 처럼 보인다(세션 407 실측, 진짜 결함 아님·상대경로 문제). ⚠ 작업반 보고를 옮기지 말고 **매번 직접 재실측** — 세션 408 이 1848 로 잘못 보고했다(테스트 추가 후 미측정), [[feedback-baseline-number-needs-own-measure]]. 옛 문서값 1863·1845(s407~408)·1823(s406)·1804(s403)·1800(s402)) |
