      ...
      <div className="flex items-center gap-0.5 justify-between">
        ...
        {column.filterType && (
          <button
            ...
            className={`ml-0.5 text-[10px] leading-none px-0.5 rounded hover:bg-gray-200 ${isFiltered ? "text-blue-600" : "text-gray-400"}`}
            title="필터"
          >
            ▼
          </button>
...
function FilterDropdown({
  ...
  return (
    <div
      className="absolute top-full left-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg z-50 min-w-[160px] p-2"
...
function CategoricalFilter({
  ...
  const toggleAll = () => {
    if (allSelected) {
      onFilterChange(columnKey, { type: "categorical", selected: new Set() });
    } else {
      onFilterChange(columnKey, undefined);
    }
  };
  ...
  return (
    <div className="max-h-[200px] overflow-y-auto space-y-1">
      ...
      {uniqueValues.map((v) => (
        <label key={v} className="flex items-center gap-1.5 text-[11px] text-gray-700 cursor-pointer">
    ...
    <div className="space-y-1.5">
      <div className="flex items-center gap-1">
        <input
          ...
          className="w-[70px] text-[11px] border border-gray-200 rounded px-1.5 py-1"
        />
