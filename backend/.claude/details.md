...
- 한계·비용: 회차 시간 예산 **120분**(단지 사이에서만 검사 — 최악 120 + 마지막 단지 17.6분 + 카나리 15분 ≈ 152분 < 스윕 3h. 옛 150분은 09-25 오후 실측 154.6분). 카나리 표본 4건은 논리 호출이고 재시도 포함 최대 16콜·표본당 43초(이전 달 표본은 연속 실패 재확인 경로에서만 — 전량 빈 응답 가드는 최신 3건 그대로). `kapt_match` 경로(목록·기본정보)는 **일시 오류도 재시도하지 않는다**(`_body` → `retry_transient=False`, 실패 1건 = 1콜·즉시 None — 장애일에 기본정보 14,747건이 4콜씩·대기 176시간이 되던 것). 수집 ≥1 이고 **실패 ≤ 수집**이면 completed 에 "N단지 호출 실패(마지막 사유: …)" 를 남기고, **실패 > 수집**이면 failed(`mostly_failed`, "실패 N단지 > 수집 M단지 — 대부분 오류") — 한 단지만 성공해도 completed 면 monitor 가 직전 실패를 "복구" 로 풀어 버렸다. 이미 running 인 `kapt_costs` 가 있으면 새 잡 행 없이 `already_running` 으로 반환(관리자 버튼 이중 실행 방지). 재시도 INFO·"재시도 3회 후" WARNING·예외 메시지에 `kaptCode=… searchDate=…` 가 붙어 "특정 달·단지만 고장인가" 를 로그로 가른다(알림에는 `explain_error` 가 사유 번호 문장으로 바꿔 영문 키가 안 나간다).
