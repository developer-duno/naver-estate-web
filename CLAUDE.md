...
**마지막 작업**: 2026-05-06 — 세션 112, **1세대1주택 5종 특례주택 출시 (PDF #12, 5종 라디오 5칸 + 카테고리별 채수+공시가 분리 입력)**. 6 커밋 push (ad85c6d Phase A-1 산식 본체+Notice 5 신규 / 05eab2b Phase A-2 회귀 가드 18 신규+단위 정정 / 7d780c5 Phase B-1 AdvancedFields 6번째 블럭+Inputs+Calculator / d6d190a Phase B-2 ResultCard 5종 표시 행 / fc7953f Phase C ResultCard 4 케이스+e2e 1 케이스 / 21ad779 Phase D mdx+realestate-calculators+ASSETS). vitest **1010 → 1032** (+22: 12 special-houses + 6 rules + 4 ResultCard). 사용자 가치 🟢 34~35 → **35~36건째** (한국 1주택자 절세 핵심 시나리오 5종 자동 산정). **plan v1 폐기 → v2 재작성**: 적대적 비판 11건 (🔴 5 + 🟡 6) + 9 GATE v1 🟡 5건 발견 후 PDF #12 단독 출시로 재설계 (PDF #13 다음 세션 이월). 핵심 결함 정정: H-1 (PDF #13 4종 거짓 박제 — 실제 4종은 상속/무허가/소형신축/준공후미분양, PDF #12에 없는 소형신축 누락 발견) + M-1 (1주택 vs 3주택 모순, PDF #12 + #13 동시 활성 불가능) + S-1 (사용자 공시가 모름) + CI-1 (단일 테스트 +180줄 100줄 룰 위반 → 신규 파일 분리) + H-3 (안분 산식 분모 단위 본문 5중 fetch 차단). **PDF 본문 직접 검증**: PDF #12 페이지 1~4 (5종 자격 + 부부 자격 표 + 사후관리) + PDF #2 페이지 4 (산식) + PDF #13 페이지 1~3 (4종 차이 확인). **종부세법 §9 안분 분모 단위 미확인** (lawnb·국가법령정보센터·국세청·법제처·moef 5중 fetch 차단) → 공시가 비율 단순 박제 + mdx 면책 강화 (분모 단위 해석 차이 가능, 실무상 ≤1% 오차). **5종 카테고리 박제값**: ① 신규주택(일시적2주택, 신규 취득 ≤3년) / ② 상속주택(상속개시 ≤5년 또는 지분 ≤40%/공시가 ≤6억(수도권 밖 3억) 포함) / ③ 지방저가(공시가 ≤4억 + 수도권 밖) / ④ 인구감소지역(공시가 ≤4억 + 인구감소지역 + 2024~2026.12.31 + 기존 1주택과 다른 시·군·구) / ⑤ 준공후미분양(전용 ≤85㎡ + 취득가 ≤6억 + 수도권 밖 + 2024.1.10~2025.12.31 + 양도자 사업주체/시공자 + 시·군·구청장 확인). **안분 산식**: 세액공제 = comprehensiveTaxAfterPropertyCredit × (1주택 공시가 / (1주택 공시가 + 5종 합계 공시가)) × 공제율 (한도 80%). **양립 가드 (normalize)**: 법인 차단 + houses≠1 차단 + 자격 미충족 차단 + 카테고리별 count 0~3 clamp + count=0 시 publishedTotal=0 강제 + B-5 우선 양립 (안분 비활성). **Phase A-1**: types.ts SpecialHousesInput + SpecialHouseEntry + PropertyTaxInput.specialHouses? + NoticeKey +5 키 / property-tax-rules.ts normalize 분기 (SPECIAL_HOUSE_KEYS 반복 처리 + normalizeSpecialHouseEntry 헬퍼) / property-tax.ts isSingleSpecialHouseEligible + isSingleComprehensive 조건 확장 + 안분 산식 적용 / Notices 5 신규 항목 (PDF #12 본문 인용 + 자격 본인 책임 + 분모 단위 면책). **Phase A-2**: property-tax-special-houses.test.ts 신규 파일 (12 케이스 #SH-1~#SH-12) + property-tax-rules.test.ts +6 신규. 단위 정정 박제 (publishedTotal 만원 단위, 50_000 = 5억). **Phase B-1**: AdvancedFields.tsx 6번째 fieldset + nested details 5종 자격 안내 + 5 카테고리 grid (라벨 + 채수 select 0~3 + 공시가 input) + 3중 disabled 가드 + count=0 시 공시가 input disabled / Inputs.tsx Props 1개 추가 / Calculator.tsx useState + handleSpecialHouseEntryChange useCallback + useMemo deps. **Phase B-2**: ResultCard.tsx showSpecialHouses + count > 0 카테고리만 표시 (행 폭증 방지) + 안분 비율 안내 + Calculator props 전달. **Phase C**: ResultCard 테스트 4 케이스 + e2e 케이스 11 (PDF #12 통합 흐름) — R1 strict mode 사전 grep 답습으로 "안분 비율" 2곳 매칭 정확 매처 (콜론·prefix 분리). **Phase D**: blog property-tax-guide.mdx '## 1세대1주택 5종 특례주택 (PDF #12, 세션 112 출시)' 섹션 신설 (5종 자격 표 + 안분 산식 + 부부 자격 표 + 사후관리 + 신청 절차 + 본 도구 처리 범위 ✅5/❌2) + realestate-calculators.mdx 1줄 갱신 + ASSETS.md PDF #12 alias 5 키 + §6 운영 부채 해소 표기. tsc 0 / lint 0 errors / vitest 1032. **plan**: `C:\Users\user\.claude\plans\112-cwd-deep-haven.md` (v2, 9 GATE 🟢8/🟡1(권위 출처 분모 단위 의도적 인정)/🔴0 + 적대적 비판 11건 모두 해소). **다음 세션 후보**: PDF #13 출시 (4종 라디오 BRACKETS_3 → BRACKETS_2, ~3시간) / 공동명의 + 5종 안분 분자 자동화 (~2시간) / complex-visual baseline 부채 해소 / ASSETS·GLOSSARY drift 자동화 hook.

**과거 작업 (세션 111)**: **법인 9종 일반 누진세율 특례 출시 (PDF #14 페이지 2 표 직접 인용 9 카테고리 라디오)**. 4 커밋 push (73e2720 Phase A+B-3 통합 산식 분기+Notices 거짓 정정 / 047c915 Phase B-1+B-2+B-4 select UI+ResultCard 입력값 표시 / 8fecd3e Phase C 8 신규 케이스+Phase A 빈틈 fix (effectiveHouses+합산배제 양립) / 8201241 Phase D mdx+ASSETS.md). vitest **997 → 1010** (+13: 8 신규 #CGR-1~#CGR-8 + 5 회귀 가드 강화). 사용자 가치 🟢 32~33 → **34~35건째** (법인 9종 누진 정확 산정). **권위 출처 100% 검증**: PDF #14 (`국세청법인 주택분 일반 누진세율 특례.pdf`) 페이지 2 표 + PDF #2 (`종합부동산세 법령 안내자료.pdf`) 페이지 3·4 직접 본문 Read 인용. 검색 결과·TAXLY 2022·LBOX 의존 0. **9 GATE 풀 재검증 + 자가 점검 1+2 재실행 + plan v2.0 폐기→재작성**: ExitPlanMode 1차 거부 후 사용자 "보강하고 게이트가은 방식으로 재검증" 지시. plan v1 거짓 박제 5건 + 검증 실패 5건 = 10건 발견 → 권위 출처 재검증. 사용자 PDF 폴더 `f:/cursor/naver-estate-web/종합부동산세/` 위치 확인 (16개) → PDF #14 + #2 직접 Read 으로 거짓 정정 (Agent "7종" 진단 거짓 → PDF 표는 9종 정확 / 세션 108 Notice 거짓 박제 8개 나열 9종 표기 → 9개 정확 명단으로 동시 정정). **사용자 결정 (가)→(나)**: PDF 직접 업로드 (가) → 9종 정확 라디오 (나) 채택. **9 카테고리 박제값**: ① 공익법인등 (②미해당) 2주택 이하 BRACKETS_2 / 3주택 이상 BRACKETS_3 / ② 공익법인등 (공익목적 직접 사용) ③ 공공주택사업자 ④ 주택조합 ⑤ 정비사업시행자 ⑥ 민간건설임대사업자 ⑦ 도시개발사업시행자 ⑧ 사회적기업등 ⑨ 종중 — 모두 BRACKETS_2 일률 (다주택 보유해도 중과 안 함). **Phase A**: types.ts CorporationGeneralRateCategory union 9개 + PropertyTaxInput.corporationGeneralRateCategory? 필드 + NoticeKey corporation-general-rate-applied 추가 / property-tax.ts isCorpGeneral 분기 + 공제 9억 적용 (PDF #2 페이지 3) + multi-heavy-25e 카테고리 ① 다주택 진입 안내 / property-tax-rules.ts normalize 가드 (isCorporation=false 시 카테고리 자동 undefined 강제) / PropertyTaxNotices.tsx corporation-general-rate-applied 신규 항목 + corporation-flat-rate-applied 본문 정정 (세션 108 거짓 박제 9종 카테고리 누락 1 정정). **Phase B**: PropertyTaxAdvancedFields select 10개 (선택안함 + 9 카테고리, PDF 본문 그대로 라벨) + Calculator handleIsCorporationChange 자동 reset (UI 가드 + normalize 이중 안전망) + Inputs prop drilling + ResultCard CORP_GENERAL_LABEL 입력값 표시. **Phase C**: 8 신규 케이스 (#CGR-1 카테고리 ① + houses=1 + 30억=10,380,000원 / #CGR-2 카테고리 ① + houses=3 + 30억=10,800,000원 / #CGR-3 카테고리 ② + houses=3 일률 / #CGR-4 카테고리 ⑨ 동일 / #CGR-5 미선택 회귀 / #CGR-6 비법인 자동 undefined / #CGR-7 카테고리 ③ + 합산배제 1주택 양립 / #CGR-8 카테고리 ① + 1주택 자격 시도 12억 차단). **Phase A 빈틈 Phase C 작업 중 발견 + 동시 fix**: ① normalize에서 isCorpGeneral 시 합산배제 양립 허용 (PDF #14 합산배제 신고기간 9.16~9.30 동일) ② 카테고리 ① BRACKETS_3 진입 + multi-heavy-25e 안내를 input.houses → effectiveHouses 기준으로 정정. **Phase D**: blog property-tax-guide.mdx '법인 9종 일반 누진세율 특례' 섹션 신설 (9 카테고리 표 직접 인용) + 면책 본문 강화 + realestate-calculators.mdx 1줄 추가 + ASSETS.md §1 #14 alias 갱신 + §6 운영 부채 해소 표기. tsc 0, lint 0 errors, Frontend CI run 25400042203 success (Frontend CI 🟢 6m 7s, E2E in_progress 시점 마무리). **plan**: `C:\Users\user\.claude\plans\unified-sprouting-key.md` (v2.0, 9 GATE 🟢8/🟡1/🔴0 + 자가 점검 1+2 빈틈 0). **다음 세션 후보**: 1세대 1주택 5종 특례주택 (PDF #12+#13, 8~12시간) / complex-visual baseline 부채 해소 / ASSETS·GLOSSARY drift 자동화 hook.
