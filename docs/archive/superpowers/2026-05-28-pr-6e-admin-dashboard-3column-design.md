...
> ⚠ **폐기(세션 419, 2026-09-26, PR #584)**: 이 3열 배치(좌 목차 `AdminLeftNav` · 우 라이브 패널 `AdminLivePanel`)는
> 사장님 지적(2026-09-11 "어려운 말·배치가 난잡")에 따라 **세로 한 열 4층**(지금 상태 → 숫자 → 원인 접힘 5절 → 작업)으로
> 대체됐다. 세션 419 재조사 결과 어지러움의 실제 원인은 항상 펼쳐진 도움말 문단 11개·기간이 제각각인 실패 숫자 6곳·
> 화면의 개발자 원문 38건이었고, 3열 자체는 되돌려도 잃는 것이 거의 없었다(색점 2개만 동작·라이브 패널은 자동 갱신 없음).
> 현재 구조 = `frontend/.claude/pages-and-mb.md` §관리자 화면 구조. 아래는 역사 기록.
