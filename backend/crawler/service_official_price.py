...
# 재수집 벽시계 캡 — 현실 소실은 한 자릿수라 재수집이 수 분에 끝난다. 이론 최악은
# "대형 동 20개 × 시도 3회 전부 미구제" = 60회 조회이고, 대형 동 1회 조회가 31~42초
# (2026-09-17 라이브 실측: 대치동 48,928행 34/31/31초, 명지동 58,414행 41/42/39초)라
# **≈0.5~0.7h** 다. 즉 캡 1h 는 이론 최악을 이미 덮는다.
# ⚠ 과거 이 자리에 적혀 있던 "~1.8h"(K=1 시절)와 "~5.4h"(그것의 단순 3배)는 **둘 다
#   틀렸다** — 1.8h 는 1회 조회를 324초로 가정한 값인데 실측의 7.7배다. 수치를 고치려다
#   옛 값에 배율만 곱하면 같은 오류가 커진 채 남는다(세션 413 실사고).
# ⚠ _REPASS_MAX_ATTEMPTS 나 _REPASS_MAX_DONGS 를 올리면 **60회 × 42초 기준으로 다시
#   계산**할 것. "본 루프 최악 7h + 재수집 1h = 8h < 16h(monitor stale 예외)" 여유는
#   캡 값으로 보장되므로 캡 자체를 올릴 때만 재검토가 필요하다.
# 이로써 재수집 상한이 규모(_REPASS_MAX_DONGS)·이상(_REPASS_COLLAPSE_THRESHOLD)·시간
# 3중 방어가 된다. 단 현행 K=3 에서는 이론 최악(0.7h) < 캡(1h) 이라 **시간 축은 실질
# 여유분**이고, 실제로 발화하는 방어선은 규모·이상 둘이다.
