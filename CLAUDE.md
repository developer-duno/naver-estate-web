...
| BE pytest | `backend/tests/` | **1762개** (세션 402 실측 `--collect-only`, 크롤 구조 결함 4 PR(#505·#506·#507·#508) 전부 머지된 main 기준 — 직전 1733 대비 **+29**. 신규 파일 3개(`test_migration_v058_complex_stamps` · `test_complex_stamps` · `test_crawl_empty_error`) + `test_article_crawl_batch.py` 재작성 + scheduler/describe 보강. ⚠ 작업반 보고를 옮기지 말고 **매번 직접 재실측**, [[feedback-baseline-number-needs-own-measure]]. 옛 문서값 1733(s401)·1680(s399)) |
