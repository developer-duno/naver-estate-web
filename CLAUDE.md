...
| 영역 | 도구 | 테스트 수 |
|------|------|----------|
| FE Vitest | `frontend/src/**/__tests__/` + `frontend/scripts/__tests__/` | **2121개** (세션 395 CI 실측 run 34255542873 `2121 passed` — 세션 388 실측 2116 에 #466 crawl-status-no-store +2·#467 ArticleDetail 토큰 +3. 옛 실측 2097(세션 384)·2090(세션 369~381). 전체 실행 시 RegionSelector 1건이 리소스 경합으로 일시 타임아웃될 수 있음 — 단독 재실행 시 통과, 결함 아님) |
| FE E2E | `frontend/e2e/*.spec.ts` | **20 파일** (Playwright, --webpack 모드) |
| BE pytest | `backend/tests/` | **1614개** (세션 395 실측 — CI run 34252301158 와 로컬 전체 1회(단일 프로세스 12m24s) 동일: **1600 passed + 8 skipped + 6 xfailed** — 합산 관례(세션 393). 세션 395 시작 기준선 1588(로컬 전체 1회 1574/8/6)에 #464 +8·#465 +13·#466 +5. 이전 문서값 1530(세션 390) 대비 +84 = 50차 규칙 갱신) |
