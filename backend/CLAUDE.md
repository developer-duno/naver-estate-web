...
| `db/migrations/` | Flyway 스타일 SQL 마이그레이션 (V000~V047, 48 버전 — 최신은 하단 §DB 마이그레이션 표가 진실) |
...
- `db/migrations/` 폴더에 `V000__` ~ `V047__` SQL 파일 = 48 버전
- Supabase 에 SQLAlchemy 엔진으로 실행 (V023 = 973,837행 backfill)
- 롤백: 각 마이그레이션 파일의 역방향 SQL 실행
- 최신 = V047 (subway_stations 신규 테이블, 세션 367 — prod 적용완료·1,099행 적재·라이브 GET 검증 완료). 새 마이그레이션 시 본 표 1행 추가 의무 (`.claude/rules/release.md` 답습 — backend zombie 회피)
