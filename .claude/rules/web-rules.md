...
- 진행 상태 폴링 응답(`crawl-status`·`collect-status`·관리자 `recrawl/progress`)은 `Cache-Control: no-store` 필수. **관리자 API(`/api/admin/*`) GET 은 전부 `no-store`**, `/api/complexes/<한 세그먼트>`(정규식 `[^/]+` 라 `search`·`region` 등 하위 컬렉션 라우트도 포함 — 검색 결과가 매번 신선해지므로 무해하나 "단지 번호만"이 아님에 유의) 와 경로에 `/articles` 가 포함된 것은 `no-cache`(React Query staleTime 이 클라 캐시를 담당하므로 HTTP max-age 는 "새로 받았다고 믿는 옛값"만 만든다 — 세션 396 PR #484). 하위 경로(price-stats 등)는 3600 유지. **오류 응답(4xx/5xx)에는 어떤 Cache-Control 도 붙이지 않는다**(세션 396 사후검증: 인증 게이트 401 이 1시간 캐시돼 승인 후에도 잠긴 화면이 유지됐다) — 캐시 헤더 회귀 테스트는 반드시 **성공 응답(200)** 을 받는 상태로 단언할 것. `/api/live/` 검색 결과 max-age 를 폴링에 물려 브라우저가 3시간 캐시 → 완료 감지 불가였던 결함(세션 395)
