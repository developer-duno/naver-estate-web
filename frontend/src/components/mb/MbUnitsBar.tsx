...
export function MbUnitsBar({ value }: Props) {
  ...
  const tier = value < 100 ? "small" : value < 500 ? "mid" : "large";
  const colorClass =
    tier === "small" ? "bg-emerald-500" :
    tier === "mid" ? "bg-sky-500" : "bg-indigo-500";
  const labelClass =
    tier === "small" ? "text-emerald-600" :
    tier === "mid" ? "text-sky-600" : "text-indigo-600";
  const label =
    tier === "small" ? "소규모 단지" :
    tier === "mid" ? "중규모 단지" : "대규모 단지";
  return (
    <div className="flex flex-col gap-1">
      <div className="flex items-baseline gap-2">
        <span className="text-sm font-medium text-gray-900">{value.toLocaleString()}세대</span>
      ...
      <div
        ...
        aria-label={`세대수 ${value.toLocaleString()}세대 (${label})`}
      >
