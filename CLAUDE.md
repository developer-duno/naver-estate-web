...
**다음 우선순위 (세션 61)**:
1. 🟡 **naver_call_counter 48h+ 누적 재확인**: 2026-04-19~20 이후 재조회. 평일+주말 1사이클 확보되면 TTL 72h 상향 판단 가능 (세션 60 실측 8.5h 678회 → 세션 61 22h 1049회 확인했으나 48h 풀 사이클 미달)
2. 🟡 **시각 회귀 baseline 안정성 감시**: 현재 20장. CI 정기 실행에서 회귀 징후만 추적 (workflow_dispatch baseline 갱신 분기는 세션 56 에서 구축 완료)
3. 🟡 **tags 필터 UX 개선 후보**: 현재 `filter_options.tags` 가 단지별 **사용 중인** 태그만 반환 (적은 단지는 tags 비어있어 섹션 미노출). "인기 태그 고정 목록" 추가 검토 필요 시
4. ⚪ mibunyang 쪽 `quota_db_integration.md` 적용 (다른 프로젝트 세션)

**제거된 우선순위**: ~~admin settings 편집 모드 AdminCard~~ / ~~매물 필터 세부 확장 (반려동물/주차/즉시입주)~~ / ~~미분양 지도 뷰~~ / ~~비교 페이지 4→6~8 확장~~ — 사용자가 명시적으로 빼라고 지시 (세션 61, 세션 43 이후 6세션 동안 백로그로 끌고 다닌 부채)
