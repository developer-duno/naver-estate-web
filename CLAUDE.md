...
| BE pytest | `backend/tests/` | **1754개** (세션 402 실측 `--collect-only`, PR #505·#506·#507 머지 시점 — 직전 1733 대비 +21. 크롤 구조 결함 4 PR 이 신규 파일 3개를 더하고 `test_article_crawl_batch.py` 를 재작성했다. #508 머지 후 다시 오르므로 그때 재실측 필요. ⚠ 작업반 보고를 옮기지 말고 **매번 직접 재실측**, [[feedback-baseline-number-needs-own-measure]]. 옛 문서값 1733(s401)·1680(s399)) |
