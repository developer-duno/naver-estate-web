...
// @ts-nocheck
/**
 * 네이버 공식 마커 클러스터링 (Apache 2.0, Copyright 2016 NAVER Corp.)
 * 원본: https://github.com/navermaps/marker-tools.js/blob/master/marker-clustering/src/MarkerClustering.js
 * react-naver-maps 공식 예제가 그대로 쓰는 소스와 동일:
 * https://github.com/zeakd/react-naver-maps/blob/main/website/src/samples/marker-cluster.ts
 *
 * 별도 npm/CDN 배포가 없어(2020-02-14 최종 커밋, 이후 미유지보수) 소스를 프로젝트에
 * 직접 벤더링. react-naver-maps 채택 결정(계획 문서 §핵심결정2)과 별개로, 클러스터링
 * 알고리즘 자체는 여전히 이 네이버 공식 구현을 감싸 쓴다 — react-naver-maps는 지도
 * 렌더링/SDK 로딩/React 통합만 담당하고 클러스터링은 자체 구현하지 않기 때문(조사 확인).
 *
 * naver.maps.OverlayView 를 상속하는 구식(ES5, Util.ClassExtend) 방식이라 타입 체크
 * 대상에서 제외(@ts-nocheck) — 원본 구조를 그대로 보존해 향후 upstream 변경과 diff
 * 비교가 쉽도록 유지.
 */
...
type MarkerClusteringConstructor = new (
  options: MarkerClusteringOptions,
) => MarkerClusteringInstance;

export function makeMarkerClustering(
  ...
  return MarkerClustering as unknown as MarkerClusteringConstructor;
}
