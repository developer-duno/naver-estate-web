...
- 무엇: `fetched_at < 2026-09-24 16:02Z`(#572 라이브 반영 시각) 행을 id 순으로 돌며 다칸 op 5종만 다시 받아 칸 합으로 갈아 끼우고 요약 4칸·`fetched_at` 을 고친다(나머지 17키 불변, 개별 옛 이름 `…V2` 행도 공용/개별 구분 유지). 비어 온 op 는 옛 값 유지(단 5 op **전부** 비면 이상 신호로 보고 갱신 안 함·`all_blank` 계수, 연속 10행이면 종료), 한 op 라도 호출 실패면 그 행은 건너뜀(반쪽 갱신 금지), 한도 초과면 즉시·연속 실패 10행이면 종료. 고친 행은 기준 뒤로 넘어가 재실행이 남은 행부터 이어간다.
- 인자: `--dry-run`(콜 0 — 대상 수·예상 콜·첫 10행) · `--limit N`(조회 행 상한 — 건너뛴 행도 센다) · `--daily-cap N`(기본 45,000) · `--sleep-between 초` · `--force`.
- 하루 상한·실행 창: 10,521행 × 5 = 52,605콜이라 이틀 분할 — `quota:kapt:<KST 날짜>` 가 `--daily-cap` 에 닿으면 정상 종료. ⚠ 기본 45,000 은 **그날 정기 06:20 회차가 이미 돈 뒤** 전제라 **09:00 이후 시작, 자정 전 종료**(자정을 넘기면 다음 날 정기 회차 몫을 먼저 먹는다). 06:20~09:00 KST 와 `kapt_costs` running 중에는 시작 거부(돌던 중 06:20 에 닿거나 100행마다 다시 본 `kapt_costs` 가 running 이어도 멈춤, 창만 `--force` 로 무시).
