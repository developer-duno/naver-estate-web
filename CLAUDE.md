...
| FE E2E | `frontend/e2e/*.spec.ts` | **20 파일** (Playwright, --webpack 모드. 시각회귀 baseline PNG **21장**(세션 417 PR #568 에 iPhone 13 뷰포트 project `public-visual-mobile` 의 `mibunyang-public-visual-mobile-linux.png` 1장 추가, chromium 엔진·배율 1 — 세는 법 = `find frontend/e2e -name "*.png" | wc -l` — 헤더 전용 `header-public-desktop` baseline 은 PR #501 에서 생성·커밋됐다. 옛 "19장·생성되면 20장" 표기는 세션 412 검사관이 실측으로 정정). 재생성 순서·판정·함정 = `frontend/e2e/README.md` §baseline 재생성 절차(세션 412 에 testing.md 에서 이동 — 시각회귀 절차 정본). project 별 대응표도 같은 파일) |
