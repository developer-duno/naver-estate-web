...
> `.claude/rules/infra.md` §스케줄러 표의 여섯 잡(매물 상세 보강(매물 상세 내용 채우기)·공동주택 공시가격·응급의료·K-apt 단지 매칭(관리비 단지 연결하기)·
> data.go.kr API 버전 감시(정부 자료 창구 살아있나 확인)·크롤링 모니터(서버 일감 점검))을 파고들 때 읽는 원문. 표는 훑어보는 용도, 여기는 파고드는 용도.
...
> 새 이름(관리자 화면·알림, 세션 419 #585): **매물 상세 내용 채우기**
...
> 새 이름(관리자 화면·알림, 세션 419 #585): **관리비 단지 연결하기**
...
> 새 이름(관리자 화면·알림, 세션 419 #585): **단지 관리비 받기**
...
- 한계·비용: 회차 시간 예산 **120분**(단지 사이에서만 검사 — 최악 120 + 마지막 단지 17.6분 + 카나리 15분 ≈ 152분 < 스윕 3h. 옛 150분은 09-25 오후 실측 154.6분). 카나리 표본 4건은 논리 호출이고 재시도 포함 최대 16콜·표본당 43초(이전 달 표본은 연속 실패 재확인 경로에서만 — 전량 빈 응답 가드는 최신 3건 그대로). `kapt_match` 의 **기본정보(`fetch_apt_basis_info`)는 일시 오류도 재시도하지 않는다**(`retry_transient=False`, 실패 1건 = 1콜·즉시 None — 장애일에 14,747건이 4콜씩·대기 176시간이 되던 것). **목록(`fetch_apt_list_page`, 약 22페이지)은 재시도를 유지**한다 — 04 한 번에 목록이 끊기면 일부 목록으로 매칭돼 `_clear_conflicting_mappings` 가 멀쩡한 매핑·관리비 행을 지울 수 있다. 수집 ≥1 이고 **실패 ≤ 수집**이면 completed 에 "N단지 호출 실패(마지막 사유: …)" 를 남기고, **실패 > 수집**이면 failed(`mostly_failed`, "실패 N단지 > 수집 M단지 — 대부분 오류") — 한 단지만 성공해도 completed 면 monitor 가 직전 실패를 "복구" 로 풀어 버렸다. 이미 running 인 `kapt_costs` 가 있으면 새 잡 행 없이 `already_running` 으로 반환(관리자 버튼 이중 실행 방지). 재시도 INFO·"재시도 3회 후" WARNING·예외 메시지에 `kaptCode=… searchDate=…` 가 붙어 "특정 달·단지만 고장인가" 를 로그로 가른다(알림에는 `explain_error` 가 사유 번호 문장으로 바꿔 영문 키가 안 나간다).
- **살아있음 상한(D9)**: 연속 실패 뒤 카나리가 살아있다고 해서 계속 가는 것은 **수집 0 인 동안 2회까지**(`_ALIVE_CONTINUE_CAP_WHILE_EMPTY`) — 3번째 살아있음이면 그 자리에서 `partial_outage`("표본은 응답하지만 수집 대상은 N단지 연속 오류 — 회차 중단(살아있음 판정 2회 뒤)"). 수집이 1건이라도 있으면 상한을 보지 않는다. 09-25 14:47~17:21 수동 회차(잡 58219) 실전: 포털이 공개 단지 첫 op 에 04 를 계속 주는데 표본(보유월)은 정상이라 "계속" 이 15번(약 10분 간격) 반복돼 **수집 0 · 실패 75 · 미공개 162 · 1,276콜(논리 691 + 재시도 585) · 150분** 을 태웠다. 상한이 있었다면 3번째 카나리(15:18:48, 시작 후 약 31분)에서 멈춰 실패 15 · 미공개 약 32 · 약 255콜(1,276 × 3/15 비례 추정)로 끝났다.

**기존 행 재수집 스크립트** (`scripts/recollect_kapt_5ops.py`, 세션 417 후속 — 위 ⚠ "재계산 불가" 의 처방)
- 무엇: `fetched_at < 2026-09-24 16:02Z`(#572 라이브 반영 시각) 행을 id 순으로 돌며 다칸 op 5종만 다시 받아 칸 합으로 갈아 끼우고 요약 4칸·`fetched_at` 을 고친다(나머지 17키 불변, 개별 옛 이름 `…V2` 행도 공용/개별 구분 유지). 5 op 중 하나라도 비어 오면 갱신 안 함(일부만 비면 `partial_blank`, **전부** 비면 `all_blank` 계수 — 연속 10행이면 종료, 아래 실행 창 줄 참조), 한 op 라도 호출 실패면 그 행은 건너뜀(반쪽 갱신 금지), 한도 초과면 즉시·연속 실패 10행이면 종료. 고친 행은 기준 뒤로 넘어가 재실행이 남은 행부터 이어간다.
- 인자: `--dry-run`(콜 0 — 대상 수·예상 콜·첫 10행) · `--limit N`(조회 행 상한 — 건너뛴 행도 센다) · `--daily-cap N`(기본 45,000) · `--sleep-between 초` · `--force`.
- 하루 상한·실행 창: 10,521행 × 5 = 52,605콜이라 이틀 분할 — `quota:kapt:<KST 날짜>` 가 `--daily-cap` 에 닿으면 정상 종료. ⚠ 기본 45,000 은 **그날 정기 06:20 회차가 이미 돈 뒤** 전제라 **09:00 이후 시작, 자정 전 종료 — 코드가 09:00 전 시작을 막고 자정에 멈춘다**(자정을 넘기면 다음 날 정기 회차 몫을 먼저 먹는다). 시작 허용 = 09:00~23:59 KST(그 전은 `refused_window`, `--force` 로만 무시), 돌던 중 KST 날짜가 바뀌면 `date_rollover` 로 정지(`--force` 로도 안 풀림), `kapt_costs` running 중에는 시작 거부·100행마다 다시 봐 running 이면 멈춤. 5 op 중 **하나라도** 비어 오면 그 행은 갱신하지 않고 `partial_blank` 로 세며 다음 실행 대상에 남긴다(저장 행은 22키 전부 아니면 전무라 부분 빈 응답은 이상 신호 — 옛 "비어 온 op 는 옛 값 유지" 는 첫 칸 값과 칸 합이 한 행에 섞였다).

### 잡 상세 — data.go.kr API 버전 감시

> 새 이름(관리자 화면·알림, 세션 419 #585): **정부 자료 창구 살아있나 확인**
...
> 새 이름(관리자 화면·알림, 세션 419 #585): **서버 일감 점검**
...
- **처방**: 여러 PR을 연속 배포할 때 매 PR마다 재시작하지 말고, 가능하면 **묶어서 한 번에
  ...
  단위로 해도 된다). 부득이 짧은 간격으로 여러 번 재시작해야 하면, 크롤링 모니터(서버 일감 점검) 텔레그램에
