...
**마지막 작업**: 2026-04-10 — 세션 32 완료 (Named Tunnel 완전 설정 + MCP 정리)

**다음 우선순위**:

1. Gmail 앱 비밀번호 설정 → SMTP_PASS 교체 → 이메일 발송 테스트
2. 4/11(토) collect_public_trades 수집 결과 확인 (/admin → SchedulerMonitor)
3. 어린이집 수집 수동 트리거 (/admin → 어린이집 버튼, cpmsapi030 전환 후 첫 수집)
...
| 영역 | 도구 | 테스트 수 |
|------|------|----------|
| FE 단위/컴포넌트/훅/페이지 | Vitest | 529개 (59파일) |
| E2E | Playwright | 48개 (9파일, --webpack 모드) |
| BE 단위/통합/API | pytest | 444개 (36파일, 1 skipped) |
