...
const MAX_COVERAGE = 80;

export function MbCoverageBar({ value }: Props) {
  if (value == null || !Number.isFinite(value)) {
    return <span className="text-sm text-gray-400">-</span>;
  }
  const capped = Math.min(Math.max(value, 0), MAX_COVERAGE);
  const widthPct = (capped / MAX_COVERAGE) * 100;
  const tier = value <= 50 ? "safe" : value <= 65 ? "warn" : "danger";
  const colorClass =
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <div
        ...
        aria-valuemin={0}
        aria-valuemax={MAX_COVERAGE}
        aria-label={`건폐율 ${value.toFixed(1)}% (${label})`}
      >
