...
(세션 208 — 1h 는 오히려 오탐이라 낮춘 값, **되돌리지 말 것**) 방금 시작한 잡은 스윕에도 안 잡혀
`running` 으로 **영구 고착**된다. 모니터는 1h 뒤에야 `crawl_stale` 경보를 내고, 그 전에 사람이
손으로 정리해야 한다.

- 순서: (1)(2) 확인 → **곧바로** `Restart-Service`(같은 분 안에). 사이에 다른 일을 했으면 (1) 부터 다시.
- 이미 끊었으면(재시작 뒤에도 `status='running'` 인데 프로세스는 새것): 그 잡 **한 건만** 수동 정리 —
  ```sql
...
| 409 (2026-09-17) | (무피해, 절차 결함) §3-0 (1) 을 재시작 **4분 전**에 확인하고 그대로 믿은 채 02:24 재시작 → 그 사이 02:21:49 에 시작한 `article_detail`(#53918) 이 끊김. 부팅 스윕은 시작 5분 넘은 잡만 정리해 그 잡은 `running` 으로 남았고 02:30 수동 cancelled 처리(경보 미발화). 5분 임계 자체는 세션 208 근거로 유지 | §3-0 에 "직전 재조회(1분 룰)" + 고착 잡 수동 정리 SQL 추가 |
