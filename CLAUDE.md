...
**마지막 작업**: 2026-05-02 — 세션 95, **/blog/realestate-calculators 본문 발행** (placeholder → draft:false). 2커밋 push (9912cd0 Phase A 본문 66줄 전면 재작성 / [TBD] Phase B+C posts.ts draft 해제 + blog.test.tsx 갱신 + 신규 케이스 + CLAUDE.md). 본문 구조 jeonse-ratio.mdx 패턴 모방(이끔말+로드맵표+1순위출시+사용흐름3단+다음출시+면책blockquote+자료출처3 외부링크). 출시 로드맵 표 1순위 "✅ 출시 완료 (2026-05-02)" + /tools/brokerage-fee 내부링크. posts.ts 한 항목 4줄 갱신: title `(준비 중)` → `1순위 중개수수료 출시`, description 출시 톤, readingTime 3→5, draft:true 제거. **자동 반영(코드 수정 0)**: sitemap.ts `POSTS.filter(!draft)` 1행 추가 / generateMetadata robots:noindex 자동 해제 / PostCard `<Link>` 자동 활성화. blog.test.tsx 2 케이스 갱신(하드코드 4→draftCount 동적, 1편→publishedSlugs 동적) + 신규 1 케이스(realestate-calculators 메타 발행 톤 검증). vitest 761 → 762. **Phase B+C 합치기 결정**: 사용자 합리성 질문 시 plan은 분리였으나 CI 빨간불 회피가 revert 단위 명확성보다 가치 큼 → 2커밋으로 압축. 100줄/3파일 룰 안전 마진(posts.ts 4 + blog.test.tsx ~10 + CLAUDE.md ~10).
