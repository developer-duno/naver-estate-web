...
_STALE_HOURS_BY_TYPE = {
    # ⚠ 3h 는 관측 최대(262분=4.4h)보다 작아 이미 부족했다(2026-09-14 실측).
    #    관측의 ~2배인 8h 로 상향 — kapt_match 4h→8h 와 같은 사유.
    "public_trade_data": 8,
    "official_price": 16,
    ...
    "childcare": 3,
    # 상세 백필(세션 402): 00:20 배치 1500 ≈ 38분 / 12:20 배치 4000 ≈ 100분 설계.
    # ⚠ 첫 실전(2026-09-14 12:20)에서 **이 표에 없어 기본 1h 로 판정돼 66분에 sweep**
    #    당했다 — official_price(세션 369)·kapt_match 와 **똑같은 오탐 sweep 사고**가
    #    새 잡에서 그대로 재발한 것이다. 배치가 4000 으로 고정이라 매일 같은 지점에서
    #    죽는다(취소 전까지 채운 데이터는 커밋돼 손실은 없으나 회차가 늘 미완주).
    # 관측 최대(100분)의 ~2배 여유로 4h. 배치를 더 키우면 이 값도 함께 올린다.
    "article_detail_backfill": 4,
    # 시세 이력 소급(매일 03:30): 평균 6분이나 **관측 최대 354분(5.9h)** — 대상 단지가
    # 몰린 날 국토교통부 API 를 길게 훑는다. 표에 없어 기본 1h 로 판정돼 90일간
    # monitor 가 3회 cancelled 시켰다(2026-09-14 실측). 관측의 ~2배인 12h.
    "price_backfill": 12,
    # 시세 이력 수집(수요일 04:00): 관측 최대 38분이나 1h 경계라 보수적으로 3h.
    "price_history": 3,
}
