...
무엇을 하나
    ...
    - 어떤 op 가 비어 오면(미공개 모양·칸이 전부 null) **그 op 의 옛 값을 유지**하고 로그를 남긴다.
      0 으로 덮지 않는다 — 없는 값을 0원으로 저장하면 총액이 더 틀려진다.
...
def in_blocked_window(now_kst: datetime) -> bool:
    start, end = BLOCKED_WINDOW
...
def run(db, *, limit: int | None = None, daily_cap: int = DEFAULT_DAILY_CAP,
    ...
    if not force and in_blocked_window(now_fn()):
        ...
        logger.error("06:20~09:00 KST 는 정기 관리비 수집 시간이라 시작하지 않는다(--force 로만 무시)")
        return stats
    ...
    for index, target in enumerate(targets):
        ...
        for op in kept:
            logger.info("행 %s(단지 %s·%s) %s — 비어 와서 옛 값 유지",
...
def dry_run(db, daily_cap: int = DEFAULT_DAILY_CAP) -> dict:
    ...
    print(f"[dry-run] 정기 수집 running: {kapt_costs_running(db)} · 지금 KST {datetime.now(KST):%H:%M}"
          f" · 차단 창 {BLOCKED_WINDOW[0]:%H:%M}~{BLOCKED_WINDOW[1]:%H:%M}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--force", action="store_true", help="06:20~09:00 KST 창 거부를 무시")
