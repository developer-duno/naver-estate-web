...
**마지막 작업**: 2026-05-02 — 세션 94, **중개수수료 계산기 (/tools/brokerage-fee)** 신설. 4커밋 push (3d64532 Phase A 순수 로직+25 단위 / 472ae9b Phase B-1 페이지+컨테이너 inline placeholder / 6f3d999 Phase B-2 입력 폼+결과 카드 / f627857 Phase C sitemap+Header+8 통합). 신규 7파일(brokerage.ts 98 + brokerage-brackets.ts 33 + brokerage-format.ts 58 + page.tsx 46 + BrokerageCalculator.tsx 57 + BrokerageInputs.tsx 96 + BrokerageResultCard.tsx 80 + brokerage-fee.test.tsx 78) 수정 2파일(sitemap +1줄 priority 0.8 / Header +1줄 navLinks "계산기"). 시행규칙 별표1 4종 요율(주택매매·임대 6구간 / 오피스텔 주거용 0.5/0.4 / 오피스텔 비주거+주택외 0.9% 협의) + 월세환산(×100→×70 재적용) + validateAmount(1조원·Infinity·음수 방어) + 부가세 일반/간이. **9 GATE 2회 풀 검증** (3 에이전트 병렬 + 메인) — 1차 🔴 5건(aria/극단값/입력초기화/B-1B-2 빌드/협의형 박스) 후 2차 🔴 0건 진입. 만원 입력 → ×10000 원 변환, fieldset+legend, HintIcon 재사용 (오피스텔 주거용 도움말), aria-live=polite 결과카드, min-h-[44px] 터치, 거래유형 변경시 입력 0 초기화 / 매물·부가세 유지. 1차 소스 4곳(법제처/서울시/경기도/창원시) 교차검증. **BrokerageCalculator placeholder 패턴**: B-1에서 B-2 자식 컴포넌트 없이 빌드 호환되도록 더미 박스 2개 → B-2에서 import + 자식 호출 4줄로 교체. vitest 728 → 761 (+33). **CI 사고 예상**: Header navLinks +1줄로 admin/public baseline 20장 깨질 가능성, 깨지면 workflow_dispatch baseline 갱신 1커밋 후속.

**100줄 룰 사고 회피**: brokerage.ts 단일 파일 시도 시 202줄 → 100줄 룰 위반 → brokerage-brackets.ts(상수 33줄) + brokerage-format.ts(buildResult 헬퍼 58줄) + brokerage.ts(진입점 98줄) 3파일로 사전 분할. BrokerageInputs도 103줄 → 96줄로 사후 압축. 4커밋 모든 파일 100줄 미만 안전 마진.
