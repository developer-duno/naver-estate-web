...
| `useSessionToken` | Supabase 세션 토큰 + `tokenReady` — 승인 전용 API(`getArticles`·`getPriceStats`·`getPyeongDetails` 등) 호출부는 **반드시** 이 토큰을 넘기고 `enabled: tokenReady` 로 게이트. `page.tsx` 와 `ArticleDetail`(모달, 자식 MarketPosition·CompetingListings·MaintenanceCost 까지 props 전달 — #467 세션 395: 모달만 누락돼 B2 게이트 이후 항상 401 이던 결함) |
