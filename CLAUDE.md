...
**마지막 작업**: 2026-04-19 — 세션 64 **FE 폴링 5분 안전망 추가 — 세션 63 130bc5f 미커버 시나리오 해소** ✅ — 사용자가 세션 63 수정 후에도 "갱신 중..." 영구 비활성화 재보고. 원인: 130bc5f 의 `already_running` 분기는 **자동 크롤 중 수동 클릭** 경로만 방어했고, 이번 재발 원인은 다른 시나리오 — BE `_crawl_status` in-memory dict 가 백그라운드 스레드 예외 시 `"running"` 고정으로 누수되어 FE 폴링이 terminal 신호를 영원히 못 받음. 1 커밋 `e2ed189` 에 `MAX_POLL_ATTEMPTS=150` (2s × 150 = 5분) 안전망 + 테스트 1건 (vitest 585→586, tsc 0, lint 0). 원 플랜 E1/E2/B/C/D 5작업은 사용자 긴급 버그 재조사로 1작업 축소 (컨텍스트 절약). 사용자 가치 🟢. 부가 발견: `/api/stats` ttfb 5.5초 측정(세션 63 대비 10배 악화) — E1 캐시 작업 필요성 강화. 도메인 혼동 정정: 실 사이트는 `www.2u.pe.kr` (200 OK), `sangse.2u.pe.kr` 는 별 프로젝트. 다음 세션 우선순위: (1) 사용자 안전망 체감 확인 / (2) E1 `/api/stats` TTL 캐시 / (3) BE `_background_crawl` 예외 핸들러 보강 / (4) 태그 필터 · 모바일 이월.

**이전 기록**: 2026-04-19 — 세션 63 **갱신중 버튼 영구 비활성화 버그 수정(already_running 분기) + CI #283 Linux baseline 복구 + 성능 진단 4엔드포인트** ✅ — 세션 62 `useCrawlAction` 개편(ce1e846) 이 BE `status="already_running"` 응답 처리를 onSuccess 에서 누락. Step 1 `130bc5f` 훅에 분기 6줄 + 테스트 2건 추가 (vitest 583→585). Step 3 `c6de858` CI artifact 에서 `complex-admin-linux.png` 추출(1280×733) 로 세션 62 이후 지속된 CI fail 복구. Step 4 curl timing 실측: **`/api/stats` p50 500ms (NullPool+COUNT 병목), 튜널 오버헤드 ≈ 0**. 사용자 가치 🟢.
...
**다음 우선순위 (세션 62) — 사용자 직접 점검 우선**:

세션 61 사용자 피드백: "어려운 이야기·시스템 점검만 했고 기능 변화 와닿지 않음. 사이트 잘 돌아가는지 궁금". 따라서 다음 세션은 **사용자 사용 흐름 6항목 점검 결과 보고부터 받고 시작**:

🔴 **세션 62 첫 액션 (사용자 답변 받기)**:
1. **태그 필터 (세션 60 신규)** — sangse.2u.pe.kr 단지 매물 필터에서 #역세권 #복층 등 태그 버튼 보이고 동작하는지
2. **검색 → 단지 → 매물** 한 사이클 끊김 없이 되는지
3. **즐겨찾기 ★ 토글** 정상 작동하는지
4. **모바일** (휴대폰) 메뉴/필터 정상 표시되는지
5. **첫 진입 6초 로딩** (`/api/stats` 콜드 호출) 답답한지 — 캐시 후 1초대 정상
6. **/admin** 대시보드 단지/매물/스케줄러 다 보이는지

이 중 이상 항목을 다음 세션 최우선 작업으로 잡음. **이상 0개면 → 시스템 점검·문서화는 후순위로 미루고 실제 사용자 가치 추가 작업 (UX 개선·신기능)으로 전환**.

🟡 **백그라운드 (사용자 점검 후)**:
- naver_call_counter 48h+ 누적 재확인 (2026-04-20 이후 평일+주말 1사이클 확보 후 TTL 72h 판단)
- tags 필터 "인기 태그 고정 목록" — 사용자 점검에서 "태그 비어있는 단지 많음" 지적 시에만
- mibunyang 쪽 `quota_db_integration.md` v2 적용 (mibunyang 세션)

**제거된 우선순위**: ~~admin settings 편집 모드 AdminCard~~ / ~~매물 필터 세부 확장 (반려동물/주차/즉시입주)~~ / ~~미분양 지도 뷰~~ / ~~비교 페이지 4→6~8 확장~~ / ~~시각 회귀 baseline 정기 감시~~ — 사용자 명시 제외 또는 보험성 부채

**자기점검 규칙 (세션 62부터)**: 세션 종료 시 "이 세션이 사용자가 직접 보는 화면에 변화를 줬는가?" 자문. 3세션 연속 "아니오" 면 다음 세션 무조건 사용자 가치 작업으로 전환.
