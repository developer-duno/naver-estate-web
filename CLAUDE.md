...
| BE pytest | `backend/tests/` | **1863개** (세션 408 실측 `--collect-only`, PR #526 브랜치 기준 — 직전 1845 대비 **+18**. 알림 우리말화 3창구 회귀: 필드 사전·문구 경계(`test_field_drift_monitor.py` +9)·결제/수집 안내 분기와 조사(`test_job_error_listener.py` +6)·접두어 전수 가드(`test_plain_words.py` +4, **워크플로 YAML 포함**). 옛 서술 1845(s407)는 아래 이력 참조. 알림 쉬운 말 사전 회귀(`test_plain_words.py`) 16건 + 적대검증 반영 회귀 6건(파이썬 예외 누수·숫자 오탐·quota 맥락·트레이스백 마지막 줄·모르는 에러 이름표·렌더 전문 영문 blanket 가드). ⚠ **`--collect-only` 는 반드시 `backend/` 안에서 실행할 것** — 레포 루트에서 돌리면 `test_scheduler_monitoring_coverage.py` 가 `FileNotFoundError` 로 수집 오류를 내 "1824건 + 오류 1" 처럼 보인다(세션 407 실측, 진짜 결함 아님·상대경로 문제). ⚠ 작업반 보고를 옮기지 말고 **매번 직접 재실측**, [[feedback-baseline-number-needs-own-measure]]. 옛 문서값 1823(s406)·1804(s403)·1800(s402)·1762·1733(s401)) |
