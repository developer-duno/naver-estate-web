...
### ✅ P0-1. 자택 서버 단일 장애점(SPOF) + 감시망이 CI 예산에 종속됨 — 감시 이중화 완료 (2026-08-24 설정, 2026-08-26 작동 확인)

> **2026-08-26 세션388 재정정**: 세션387이 "테스트 알림 도착 여부 미확인"으로 격하했던 항목을,
...
- **✅ 설정 완료**: GitHub Actions와 무관한 외부 무료 서비스 **UptimeRobot**으로
  `https://api.2u.pe.kr/health/db` 5분 간격 HTTP 모니터 등록 + 이메일 알림 설정(세션 381,
  2026-08-24 — GitHub OAuth 가입 → 모니터 등록 → 테스트 알림 1회 발사까지 실행). 상세 =
  `session381_summary.md` §4.
- **✅ 작동 확인(2026-08-26)**: 사장님이 `kyh11kyh@gmail.com` 수신함 스크린샷을 직접 제공 —
  8/25일자로 **UptimeRobot 발신 "Weekly uptime report - August 2026"**(요약 기간 Aug 16~22)
  메일이 실제로 도착해 있음을 확인. 이 메일은 감시가 정상 등록·가동 중일 때만 자동 발송되는
  정기 리포트라, 세션387이 요구했던 "테스트 알림 도착 여부"보다 더 강한 증거 — 계정 연결·
  메일 발송 채널·주간 스케줄까지 살아있음을 실측으로 확정한다. 등록 당시 문서가 추정했던
  이메일 주소(`signalflower.dev@gmail.com`)는 실제 등록 주소와 달랐음(정답은
  `kyh11kyh@gmail.com`) — 채널 정정도 이번에 함께 확정.
  **UptimeRobot 대시보드 리포트 화면 직접 확인**: 모니터 대상 `api.2u.pe.kr/health/db` 정확 일치,
  전체 가동률 99.640%(8/16~22 구간), **사건(장애로 감지된 구간) 3건·총 가동중지 36분·평균
  지속 12분**이 실제로 기록됨 — 이는 "설정만 되고 잠들어 있는 감시"가 아니라 **5분 간격으로
  실제 HTTP 요청을 보내 상태를 판정하고 그 결과를 누적 기록하는 감시가 살아서 돌고 있다**는
  가장 강한 증거. 메일 발송 여부 확인을 넘어 감시 로직 자체의 가동을 실측 확정.
  **실시간 대시보드(`dashboard.uptimerobot.com/monitors`) 직접 확인**(사장님 실계정
  로그인 화면 스크린샷): 모니터 상태 "Up 1 day, 21 hr"(연속 정상 가동 중), 점검 주기
  5분 설정 그대로 활성, 최근 24시간 가동률 100%·사건 0건. 주간 리포트 메일·주간 통계에
  이어 실시간 화면까지 3중 교차 확인 완료.
- 남아있는 사업 판단(집 서버 1대 구조를 클라우드로 옮길지 여부)은 별도 항목
  (`project_predeploy_broker_audit_s311.md`) — 이 P0-1은 "감시 이중화 설정·작동"만을 가리킨다.

### P0-2. GitHub Actions 무료 한도 재소진 감시 부재 — P0-1 완료로 위험도 낮아짐, 대체 안전망 확정

- **문제**: P0-1과 같은 원인이 다른 워크플로(CI 등)로도 재발할 수 있다. `docs/ops/free-tier-budget.md`에
  "월초 대시보드 확인" 절차는 있으나 자동 알림은 없다.
- **현재 상태**: P0-1(UptimeRobot 외부 감시)이 2026-08-24 설정, 2026-08-26 작동까지 확인돼
  대체 안전망이 실증됐다. "Actions 분 급감 자체에 대한 능동 알림"은 여전히 미착수이나, P0-1이
  대체 안전망 역할을 실제로 하고 있으므로 급하지 않음.
- **권장 조치**: 월초 수동 확인(`docs/ops/free-tier-budget.md`)을 계속 유지. 능동 알림 신설은
  착수 대기 목록으로 격하(**저우선**, 재검토 불필요로 확정).
...
| P2-6 | `backend/CLAUDE.md`의 DB 마이그레이션 표·테스트 카운트 등 다른 drift 여부 정기 재점검 — **1회차 수행 완료(2026-09-03 세션 392)**: 발견 2건 = ①V052 마이그레이션 표 누락("52 버전"·"최신 V051" 표기 3곳 포함, 세션 390 이 표 1행 추가 의무를 놓침) ②kapt_api "관리비 V3/V2" 낡은 표기(PR #435 로 개별도 V3 전환 완료) → 같은 세션에서 즉시 정정. 파일 개수 주장(admin 10·db 14·formatters 5)은 전부 정확 실측. 테스트 카운트(문서 1530 vs 실측 1551)는 50차 미달 규칙상 의도된 유지 | 이번 세션에서 admin 폴더 1건 drift 발견 — 유사 패턴 가능성 | 보통(분기 1회 — 다음 2026-12월) |
...
- ~~P0-1 조치 후 1회 강제 장애 테스트~~ — ✅ 완료: P0-1 섹션 기록대로 2026-08-26 작동 확인
  (잔여 = UptimeRobot 테스트 알림 메일 도착 여부만 사장님 확인 대기, 세션 392 정합 정정).
