...
export function MbPirBar({ value }: Props) {
  ...
  const tier = value < 15 ? "safe" : value < 25 ? "warn" : "danger";
  const colorClass =
  ...
  return (
    <div className="flex flex-col gap-1">
      <div className="flex items-baseline gap-2">
        <span className="text-sm font-medium text-gray-900">{value.toFixed(1)}배</span>
      ...
      <div
        ...
        aria-label={`PIR ${value.toFixed(1)}배 (${label})`}
      >
