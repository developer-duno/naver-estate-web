...
| BE pytest | `backend/tests/` | **1839개** (세션 407 실측 `--collect-only`, PR #524 브랜치 기준 — 직전 1823 대비 **+16**. 알림 쉬운 말 사전 회귀(`test_plain_words.py`) 신설분. ⚠ **`--collect-only` 는 반드시 `backend/` 안에서 실행할 것** — 레포 루트에서 돌리면 `test_scheduler_monitoring_coverage.py` 가 `FileNotFoundError` 로 수집 오류를 내 "1824건 + 오류 1" 처럼 보인다(세션 407 실측, 진짜 결함 아님·상대경로 문제). ⚠ 작업반 보고를 옮기지 말고 **매번 직접 재실측**, [[feedback-baseline-number-needs-own-measure]]. 옛 문서값 1823(s406)·1804(s403)·1800(s402)·1762·1733(s401)) |
