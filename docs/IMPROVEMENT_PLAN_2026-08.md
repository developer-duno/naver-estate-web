...
### 🟡 P0-1. 자택 서버 단일 장애점(SPOF) + 감시망이 CI 예산에 종속됨 — 감시 이중화 설정 완료, 작동 확인 대기 (2026-08-24, 세션 381)

> **2026-08-26 세션387 정정**: 이 항목을 한때 "✅ 완료"로 표기했으나, 사후 적대적 검증에서
> 근거 문서(`session381_summary.md` 60·86행)가 **스스로 "테스트 알림 도착 여부 미확인 —
> 미완결 위임"이라고 유보**하고 있음이 확인돼 과도한 확신이었음이 드러남. "가입·등록·설정"과
> "실제로 작동하는지 확인"은 다른 단계다 — 아래처럼 구분해 정정.
...
- **🟡 미확인(진짜 잔여)**: 테스트 알림이 실제로 메일함(스팸함 포함)에 도착했는지는 Claude가
  볼 수 없어 확인 안 됨 — `project_backlog_survey_2026_08_24.md` §8 참조. **이게 확인되기
  전까지는 "5분 내 장애 감지 가능"이 검증된 사실이 아니라 설정상의 기대치**다. 사장님이
  메일함을 확인해주셔야 이 항목이 진짜 완료로 격상된다.
- 남아있는 사업 판단(집 서버 1대 구조를 클라우드로 옮길지 여부)은 별도 항목
  (`project_predeploy_broker_audit_s311.md`) — 이 P0-1은 "감시 이중화 설정"만을 가리킨다.

### P0-2. GitHub Actions 무료 한도 재소진 감시 부재 — P0-1 설정 완료로 위험도 낮아짐 (단 작동 미확인 전제)

- **문제**: P0-1과 같은 원인이 다른 워크플로(CI 등)로도 재발할 수 있다. `docs/ops/free-tier-budget.md`에
  "월초 대시보드 확인" 절차는 있으나 자동 알림은 없다.
- **현재 상태**: P0-1(UptimeRobot 외부 감시)이 2026-08-24 설정 완료돼 대체 안전망이 가동 중이다 —
  단 위 P0-1의 "미확인" 항목대로 실제 알림 도착이 아직 검증 안 됐으므로, "확실히 안전해졌다"가
  아니라 "설정상 안전망이 생겼다"는 잠정 상태다. "Actions 분 급감 자체에 대한 능동 알림"은
  여전히 미착수이나, P0-1이 (검증되면) 대체 안전망 역할을 하므로 급하지 않음.
- **권장 조치**: 월초 수동 확인(`docs/ops/free-tier-budget.md`)을 계속 유지. 능동 알림 신설은
  착수 대기 목록으로 격하(**저우선**). P0-1 알림 도착이 확인되면 이 항목도 재검토 불필요로 확정.
