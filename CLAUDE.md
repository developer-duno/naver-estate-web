...
| BE pytest | `backend/tests/` | **1804개** (세션 403 실측 `--collect-only`, main `5466998` 기준 — 직전 1800 대비 **+4**. #516 감시 회귀(`test_field_drift_monitor.py`) + #520 오탐 sweep·CI 가드(`test_monitor.py`·`test_scheduler_monitoring_coverage.py`) 보강분. 세션 402 의 1800 은 그 두 PR **머지 전** 시점 값이라 낡은 것이지 이상 신호가 아니다. ⚠ 작업반 보고를 옮기지 말고 **매번 직접 재실측**, [[feedback-baseline-number-needs-own-measure]]. 옛 문서값 1800(s402)·1762·1733(s401)·1680(s399)) |
