...
const MAX_UNSOLD = 100;

export function MbUnsoldRateBar({ value }: Props) {
  if (value == null || !Number.isFinite(value)) {
    return <span className="text-sm text-gray-400">-</span>;
  }
  const capped = Math.min(Math.max(value, 0), MAX_UNSOLD);
  const widthPct = (capped / MAX_UNSOLD) * 100;
  const tier = value <= 10 ? "safe" : value <= 30 ? "warn" : "danger";
  const colorClass =
  ...
  const label =
    tier === "safe" ? "양호" :
    tier === "warn" ? "주의" : "위험";
  const overCap = value > MAX_UNSOLD;
  return (
    <div className="flex flex-col gap-1">
      ...
      <div
        ...
        aria-valuemin={0}
        aria-valuemax={MAX_UNSOLD}
        aria-label={`미분양률 ${value.toFixed(1)}%${overCap ? " (상한 초과)" : ""} (${label})`}
      >
