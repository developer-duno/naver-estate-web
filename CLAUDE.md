...
## 테스트 현황 (BE = 세션 421 CI 실측 2026-09-27 · FE = 세션 421 CI 실측 2026-09-27)

| 영역 | 도구 | 테스트 수 |
|------|------|----------|
| FE Vitest | `frontend/src/**/__tests__/` + `frontend/scripts/__tests__/` | **2320개** (세션 421 CI 실측 2026-09-27, main 6e26abc5(#604) run 36249359005 Frontend CI: Test Files 256 · Tests 2320 passed — #603 폰 스케줄러 줄·5분 캐시 안내 등 +9. 옛 2311 = 세션 419 CI 실측 2026-09-26, PR #591 run 36200974061 Frontend CI: Test Files 255 · Tests 2311 passed — 관리자 화면 리뉴얼 #584~#591 로 +79(대시보드 4층·절 펼침·수집기 8종 `collectors.test.ts`·RawDetail·hideTitle·URL 동기화 등). 옛 2232 = 세션 418 CI 실측 2026-09-25, main a140cd01(#580) Frontend CI: Test Files 248 · Tests 2232 passed — #580 엑셀 왕복 3건 등 +10. 옛 2222 = 세션 417 CI 실측 2026-09-24, PR #568 Frontend CI: Test Files 247 · Tests 2222 passed — #565 로그인 upsert 미호출 1·#568 표 min-w 2·#570 주소 1·#571 proxy 4·#573 병상 2 등. 옛 2213 = 세션 410 실측 — 2213 passed / 246 파일, 새 `it()` 0 인 PR #531 에서 잰 값이라 기준선 자체. 옛 2205(세션 401). ⚠ **옛 표기 "RegionSelector 는 단독 재실행하면 통과 = 결함 아님" 은 거짓**이었다(s399·s400 이 확인 없이 flaky 로 분류). 세션 401 에 유휴 상태 **단독 실행에서도 실패**(EXIT=1) 실측 — 사유는 단언 실패가 아니라 `Error: Hook timed out in 10000ms`(10,270ms). 원인 = `beforeEach` 가 매 테스트 `vi.resetModules()` + `@base-ui` 동적 재import 로 모듈 그래프를 6회 재구성. 전량 실행에선 통과하기도 해 회차마다 갈린다. 판정법도 정정: "단독 재실행"이 아니라 **`git diff origin/main` 으로 그 파일 무변경 확인 + 사유가 타임아웃인지 확인**. ✅ **구조 수정 완료(세션 416)** — `vi.resetModules()`·동적 재import 를 걷어내고 정적 import + `vi.hoisted` 로 바꿨다(컴포넌트에 모듈 단위 상태가 없고 `TestQueryProvider` 가 마운트마다 새 QueryClient 를 만들어 재적재가 애초에 불필요). 타임아웃이 걸리던 `beforeEach` 훅 자체가 없어졌으므로, 이후 이 파일이 타임아웃으로 실패하면 **새 원인**이다. 직전 기준선 2174 에 시각회귀 안전망 +31 신설(check-visual-guard 25 · Header 메뉴집합 5 · Footer LOCKED_PATHS 1). 옛 문서값 2148(s399)·2135(s396)) |
| FE E2E | `frontend/e2e/*.spec.ts` | **21 파일** (Playwright, --webpack 모드 — 세션 419 #589 에 `admin-dashboard-sections.spec.ts`(절 5개 펼친 대시보드) 추가. 시각회귀 baseline PNG **22장**(세션 419 실측 2026-09-26: 설정 화면 삭제로 `admin-settings-admin-linux.png` 1장 제거 · #589 `admin-dashboard-sections-admin-linux.png` 1장 · #590 iPhone 13 관리자 project `admin-mobile` 의 `admin-dashboard-admin-mobile-linux.png` 1장 추가. 세션 417 PR #568 에 iPhone 13 뷰포트 project `public-visual-mobile` 의 `mibunyang-public-visual-mobile-linux.png` 1장 추가, chromium 엔진·배율 1 — 세는 법 = `find frontend/e2e -name "*.png" | wc -l` — 헤더 전용 `header-public-desktop` baseline 은 PR #501 에서 생성·커밋됐다. 옛 "19장·생성되면 20장" 표기는 세션 412 검사관이 실측으로 정정). 재생성 순서·판정·함정 = `frontend/e2e/README.md` §baseline 재생성 절차(세션 412 에 testing.md 에서 이동 — 시각회귀 절차 정본). project 별 대응표도 같은 파일) |
| BE pytest | `backend/tests/` | **2188개** (세션 421 CI 실측 2026-09-27, main 8be1595a(#606) run 36260063729 Backend CI: 2174 passed · 8 skipped · 6 xfailed — #598 429 를 빈 달로 삼키지 않기·#600 test_max_page_limit 멈춤 제거·#601 수집 버튼 백그라운드·#602 V067 인덱스·#605 오케스트레이터 겹침 방지 +5·#606 창구 남은 횟수 +11 로 +78. 세션 421 은 다른 프로젝트 밤 배치(01:30~06:30) 중이라 로컬 전체 대신 CI 값을 썼다. 옛 2110 = 세션 419 직접 실측 `--collect-only`, 2026-09-26 08:5x, main 1bb99852 — #585 잡 이름 통일·`error_plain`·`job_type` 필터 +9 · #587 running 컷오프·CI 동기 가드 +8 로 2113 뒤, #588 설정 API 삭제로 그 테스트 3건이 빠져 2110. 옛 2096 = 세션 418 직접 실측 `--collect-only`, 2026-09-25 21:3x, main 9325c5eb — #575 봉투 재시도·카나리 +23·#577 병상 가드 +2·#579 재수집 스크립트 +23·#581 카나리 상한·mostly_failed +18 = +66. 옛 2030 = 세션 417 직접 실측 `--collect-only`, 2026-09-25 00:24 — PR #566 인덱스 가드 +1·#569 kapt 조기 탈출 +7·#570 주소 +5·#571 관리자 판정 +7·#572 관리비 합산 +~50(22항목 파라미터화)·#573 병상 +6 등 합 +84. 옛 1946 = 세션 416 직접 실측 `--collect-only`, 2026-09-19 — #548 기준 1939 → 테스트 임시 DB 경로 가드(`test_conftest_temp_db.py`) +7. 임시 SQLite 파일명에 PID 가 붙어 **같은 PC 에서 pytest 두 개를 동시에 돌려도 서로 밟지 않는다**(옛 고정 이름은 `no such table`·`WinError 32` 거짓 실패를 냈다 — 세션 415). 옛 값 1896 = 세션 412 실측(2026-09-17) — #537 기준 1889 → #539·#540 시각표 가드(`test_restart_schedule_table.py`) +7. 옛 값 1869 = 세션 410 실측 — #530 기준 1867(+2: explain_error 결제 상태별·공백 입력) → 후속 PR(마침표·마스킹·포괄 규칙 회귀 3건) 1869. 세션 409 값 1865 는 `702b01d`(#527) 기준, 1845 대비 +20. 알림 8창구 우리말화 회귀: 필드 사전·문구 경계(`test_field_drift_monitor.py`)·결제/수집 안내 분기와 조사(`test_job_error_listener.py`)·개발자 어휘/접두어 **전수 추출 가드**(`test_plain_words.py`, **워크플로 YAML 포함**). ⚠ **`--collect-only`·pytest 는 `backend/` 안에서 실행하는 것이 표준**(pytest.ini·conftest 가 그 기준). 옛 함정 = 레포 루트에서 돌리면 `test_scheduler_monitoring_coverage.py` 가 상대경로 `open("crawler/scheduler.py")` 때문에 `FileNotFoundError` 로 수집 오류를 냈다(세션 407 실측) — **세션 412 에 `Path(__file__)` 절대경로로 수정**해 이제 루트에서도 수집된다. ⚠ 작업반 보고를 옮기지 말고 **매번 직접 재실측** — 세션 408 이 1848 로 잘못 보고했다(테스트 추가 후 미측정), [[feedback-baseline-number-needs-own-measure]]. 옛 문서값 1863·1845(s407~408)·1823(s406)·1804(s403)·1800(s402)) |
