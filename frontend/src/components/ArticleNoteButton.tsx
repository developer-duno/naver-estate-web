...
/** 매물 메모 버튼 + 모달 — 매물별 손님 응대 메모 입력 (localStorage 영속) */
export default function ArticleNoteButton({ articleNo }: Props) {
  const { note, save, clear, maxLen } = useArticleNote(articleNo);
  ...
  return (
    <>
      <button
        ...
        aria-label={hasNote ? "매물 메모 보기·수정" : "매물 메모 추가"}
        title={hasNote ? `메모: ${note.slice(0, 30)}${note.length > 30 ? "…" : ""}` : "매물 메모 추가"}
      >
        {hasNote ? "📝" : "📄"}
      </button>
      {open && (
        ...
        >
          <div className="w-full max-w-md rounded-lg bg-white p-4 sm:p-6 shadow-xl">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-lg font-bold">매물 메모</h3>
              <button
                ref={closeBtnRef}
                type="button"
            ...
            <textarea
              ...
              maxLength={maxLen}
              placeholder="이 매물에 대한 메모를 입력하세요 (예: 손님 김○○ 관심, 가격 협상 가능, 이사 가능 시기)"
